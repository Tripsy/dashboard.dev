...
import type { WorkSessionsDataTableFiltersType } from '@/app/(dashboard)/dashboard/work-sessions/work-sessions.definition';
...
export const DataTableFiltersWorkSessions = (): JSX.Element => {
	...
	) as WorkSessionsDataTableFiltersType;
	...
	const setFilterValues = useCallback(
		(
			updates: Partial<{
				[K in keyof WorkSessionsDataTableFiltersType]: WorkSessionsDataTableFiltersType[K]['value'];
			}>,
		) => {
			const updatedFilters = { ...filters };

			function applyUpdate<
				K extends keyof WorkSessionsDataTableFiltersType,
			>(
				key: K,
				value: WorkSessionsDataTableFiltersType[K]['value'],
			): void {
				updatedFilters[key] = {
					...filters[key],
					value,
			...
			for (const key of Object.keys(updates) as Array<
				keyof WorkSessionsDataTableFiltersType
			>) {
				applyUpdate(
					key,
					updates[
						key
					] as WorkSessionsDataTableFiltersType[typeof key]['value'],
				);
			}
	...
	return (
		<div className="form-section flex-row flex-wrap gap-4 border-b border-line pb-4">
			<FormFiltersAutoComplete<
				WorkSessionsDataTableFiltersType,
				UserModel
			>
				labelText="User"
				fieldName="user"
			...
			<FormFiltersSelect<WorkSessionsDataTableFiltersType>
				labelText="Status"
				fieldName="status"
			...
			<FormFiltersDateRange<WorkSessionsDataTableFiltersType>
			...
			<FormFiltersReset dataSource="work-sessions" />
		</div>
	);
};
